...
export default function MaintenanceEquip() {
  ...
  useEffect(() => {
    const fetchAdminInfo = async () => {
      try {
        ...
        // If admin has a default branch, set it as the branch filter
        if (data.admin && data.admin.defaultBranchId) {
          setSelectedBranch(String(data.admin.defaultBranchId));
        }
      } catch (error) {
  ...
  // 4) Equipment (Raw) + FilteredEquipment
  ...
  // 7) Pagination States & Derived Data
  ...
  // 9) Drag & Drop with Pagination Adjustment
  ...
  // 11) Delete Confirmation & Removal
