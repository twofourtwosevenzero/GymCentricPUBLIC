...
const [newRenewal, setNewRenewal] = useState({
  ...
  const handleTabChange = (e, newValue) => {
    ...
    if (newValue !== 5) {
      setVisitDateFilter(""); // Clear visit date filter when leaving member visit logs tab
    }
    if (newValue !== 0) {
  ...
  // Add this helper function for date formatting to handle timezone consistently
  function formatDateForInput(dateString) {
    if (!dateString) return '';
    
    // Parse the date - assuming it's already in Philippines timezone
    // The server is now storing dates in PH timezone (Asia/Manila)
    const date = new Date(dateString);
    if (isNaN(date.getTime())) return '';
    
    // Format to YYYY-MM-DD without timezone offset adjustment
  ...
  function handleEditMembership(row) {
    ...
    // Format dates to YYYY-MM-DD for date input fields using the helper
    if (memberData.MembershipStartDate) {
      memberData.MembershipStartDate = formatDateForInput(memberData.MembershipStartDate);
    }
    
    if (memberData.MembershipEndDate) {
      memberData.MembershipEndDate = formatDateForInput(memberData.MembershipEndDate);
    }
    
    setSelectedMembership(memberData);
  ...
  async function handleEditMembershipSubmit() {
    ...
    try {
      ...
      // Refresh all membership data to ensure consistency
  ...
  const handleAddWalkIn = async () => {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.errors) {
        ...
        // Convert API validation errors to form validation errors
        Object.keys(validationErrors).forEach(field => {
          newErrors[field] = validationErrors[field].join(', ');
        });
  ...
  async function handleEditFreezeSubmit() {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.errors) {
        ...
        alert("Error updating freeze. Please try again.");
      }
    }
  }
  ...
  function handleOpenRenewalDialog(memberRow) {
    // Get current date with timezone adjustment if needed
    const today = new Date();
    const formattedDate = today.toISOString().split('T')[0];

    // Get the member and their plan price
    const selectedMember = getMemberByID(memberRow.MemberID);
    const planPrice = getPlanPriceForMember(selectedMember);

    // Use today's date as the renewal start date
    const renewalStartDate = formattedDate;
    
    // Calculate end date (one month later from start date)
    const endDate = new Date(today);
    endDate.setMonth(endDate.getMonth() + 1);
    const formattedEndDate = endDate.toISOString().split('T')[0];
    
    // Set initial renewal data - without userData reference
    setNewRenewal({
      MemberID: memberRow.MemberID,
      RenewalDate: formattedDate,
      RenewalStartDate: renewalStartDate,
      NewEndDate: formattedEndDate,
      RenewalAmount: planPrice, // Auto-set the renewal amount based on plan
      PaymentFor: `[${selectedMember?.PlanName || 'Membership'} Renewal]`,
      // Remove references to userData
      StaffID: 0, // This will be set by the server using the authenticated user
      BranchID: 0, // This will be set by the server using the authenticated user
    });

    // Initialize renewal payments with a single payment equal to plan price
    setRenewalPayments([
      { PaymentMethod: "", PaymentAmount: planPrice },
    ]);
    ...
    console.log("Opening renewal dialog with:", {
      memberID: memberRow.MemberID,
      planPrice,
...
// Modify the useEffect that auto-calculates renewal amount to prevent overriding initial value
useEffect(() => {
  const member = getMemberByID(newRenewal.MemberID);
  if (!member) return;
  if (!newRenewal.NewEndDate || !newRenewal.RenewalStartDate) return;

  const dtStart = new Date(newRenewal.RenewalStartDate);
  const dtChosen = new Date(newRenewal.NewEndDate);
  
  if (isNaN(dtStart.getTime()) || isNaN(dtChosen.getTime())) return;

  // Calculate months between the selected dates
  ...
  // Only update if the calculated amount is different and not zero
  if (total > 0 && total !== parseFloat(newRenewal.RenewalAmount)) {
    setNewRenewal((prev) => ({
      ...prev,
      RenewalAmount: total,
    }));
    
    // Also update the payment amount to match
    if (renewalPayments.length > 0) {
      setRenewalPayments((prev) => [
        { ...prev[0], PaymentAmount: total },
        ...prev.slice(1)
      ]);
    }
  }
}, [newRenewal.RenewalStartDate, newRenewal.NewEndDate, newRenewal.MemberID, renewalPayments.length]);
  ...
  async function handleAddRenewal() {
    ...
    // Get the total paid amount from all payments
    const totalPaid = renewalPayments.reduce((sum, p) => sum + Number(p.PaymentAmount || 0), 0);

    // Prepare the payment data correctly
    const formattedPaymentFor = Array.isArray(newRenewal.PaymentFor) 
      ? JSON.stringify(newRenewal.PaymentFor) 
      : JSON.stringify([newRenewal.PaymentFor.replace(/^\[|\]$/g, '')]);

    try {
      // Get staff branch ID
      const staffBranchID = loggedInStaff?.DefaultBranchID || null;
      ...
      const renewalData = {
        ...
        PaymentFor: formattedPaymentFor, // Properly formatted payment description
      ...
      // Send API Request
      const response = await axios.post("/membership/renewals", renewalData);

      showSuccessMessage("Renewal added successfully!");
      ...
      if (error.response?.data?.errors) {
        ...
        alert("Error adding renewal: " + (error.response?.data?.message || error.message));
      }
    }
  }
  ...
  async function handleAddMonthlyClientSubmit() {
    try {
      ...
      // Also refresh monthly client data specifically
      const monthlyClientRes = await axios.get("/monthly-clients");
      setMonthlyClientRecords(monthlyClientRes.data || []);
    } catch (err) {
      ...
      if (err.response && err.response.data && err.response.data.errors) {
        ...
        alert("Error creating monthly client. Please try again.");
      }
    }
  }
  ...
  async function handleEditMonthlyClientSubmit() {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.errors) {
        ...
        alert("Error updating monthly client. Please try again.");
      }
    }
  }
  ...
  function getFilteredData() {
    ...
    if (activeTab === 1) {
      ...
      data = applyBranchAndSearch(data);
      data = filterByDateRange(data, (w) => w.VisitDate);
      return data;
    }
    if (activeTab === 2) {
      // Renewals
      ...
      data = applyBranchAndSearch(data);
      data = filterByDateRange(data, (r) => r.RenewalDate);
      return data;
    }
    if (activeTab === 3) {
      // Freezes
      ...
      data = applyBranchAndSearch(data);
      data = filterByDateRange(data, (f) => f.FreezeStartDate);
      return data;
    }
    if (activeTab === 4) {
      // Monthly Clients
    ...
    if (activeTab === 5) {
      ...
      // Apply the single date filter
      if (visitDateFilter) {
        const filterDateStr = visitDateFilter; // Format: YYYY-MM-DD
        
        data = data.filter(visit => {
          return visit.VisitDate === filterDateStr;
        });
      }
    ...
    if (activeTab === 6) {
      ...
      data = applyBranchAndSearch(data);
      data = filterByDateRange(data, (a) => a.AttendanceDate);
      return data;
    }

    // Activity Logs
    let data = activityLogs.filter((item) =>
      Object.values(item).some((val) =>
        String(val).toLowerCase().includes(searchTerm)
      )
    );
    data = filterByDateRange(data, (log) => log.Timestamp || log.LogDate);
    return data;
  }
  ...
  const currentEndDateDisplay = currentEndDateRaw
    ...
    const getRowsForTab = () => {
      if (activeTab === 5) { // Member Visit Logs
        ...
        if (visitDateFilter) {
          data = data.filter(visit => visit.VisitDate === visitDateFilter);
          console.log("Filtering by date:", visitDateFilter);
  ...
  // Add new state for fingerprint registration
  const [isFingerprintRegistrationOpen, setFingerprintRegistrationOpen] = useState(false);
  const [selectedMemberForFingerprint, setSelectedMemberForFingerprint] = useState(null);
  
  // Add this function to handle opening the fingerprint registration dialog
  const handleOpenFingerprintRegistration = (member) => {
    setSelectedMemberForFingerprint(member);
    setFingerprintRegistrationOpen(true);
  };
  
  // Add this function to handle fingerprint registration success
  const handleFingerprintRegistrationSuccess = () => {
    showSuccessMessage(`Fingerprint registered successfully for ${selectedMemberForFingerprint.FullName}`);
  };

  // Add new state for QR code generation
  const [isQrCodeRegistrationOpen, setQrCodeRegistrationOpen] = useState(false);
  const [selectedMemberForQrCode, setSelectedMemberForQrCode] = useState(null);
  
  // Add this function to handle opening the QR code registration dialog
  const handleOpenQrCodeRegistration = (member) => {
    setSelectedMemberForQrCode(member);
    setQrCodeRegistrationOpen(true);
  };
  
  // Add this function to handle QR code generation success
  const handleQrCodeRegistrationSuccess = () => {
    showSuccessMessage(`QR code generated successfully for ${selectedMemberForQrCode.FullName}`);
  };
          ...
          <Paper elevation={2} sx={{ p: 2 }}>
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2 }}>
              <Grid container spacing={2} alignItems="center" sx={{ flexWrap: "wrap" }}>
                ...
                <Grid item>
                  {activeTab === 5 ? (
                    // Single date picker for Member Visit Logs
                    <TextField
                      label="Visit Date"
                      type="date"
                      size="small"
                        ...
                        <Grid container spacing={2}>
                          ...
                          <Grid item xs={12} sm={4}>
                            <TextField
                              ...
                              helperText="You can manually adjust this date if needed"
                            />
                          </Grid>
                          <Grid item xs={12} sm={4}>
                            <TextField
                          ...
                          <Grid item xs={12}>
                            ...
                            >
                              <QrCodeIcon color="primary" /> QR Code Check-in
                            </Typography>
                            <Box sx={{ 
                              p: 3, 
                              border: '1px dashed', 
                              ...
                              <Typography variant="body1" sx={{ mb: 2 }}>
                                Generate a QR code for this member to enable quick check-in using their mobile device.
                              </Typography>
                              <Button
                                variant="contained"
                                color="primary"
                                startIcon={<QrCodeIcon />}
                                onClick={() => handleOpenQrCodeRegistration(selectedMembership)}
                              >
                                Generate QR Code
                              </Button>
                            </Box>
        ...
        >
         ...
         <Dialog
          ...
          >
            ...
            <TextField
              ...
              margin="normal"
              variant="outlined"
              value={newRenewal.RenewalStartDate}
              onChange={(e) => {
                // Get the new start date
                const startDate = new Date(e.target.value);
                
                // Calculate end date (one month later, same day)
                ...
                // Update both start date and calculated end date
                setNewRenewal((prev) => ({ 
                  ...prev, 
                  RenewalStartDate: e.target.value,
                  NewEndDate: formattedEndDate
                }));
              }}
              InputLabelProps={{ shrink: true }}
         ...
         {/* UNFREEZE CONFIRMATION DIALOG */}
          ...
          <FingerprintRegistration
            open={isFingerprintRegistrationOpen}
            onClose={() => setFingerprintRegistrationOpen(false)}
            memberId={selectedMemberForFingerprint?.MemberID}
            memberName={selectedMemberForFingerprint?.FullName}
            onSuccess={handleFingerprintRegistrationSuccess}
          />
          {/* QR Code Registration Dialog */}
          <QrCodeRegistration
            open={isQrCodeRegistrationOpen}
            onClose={() => setQrCodeRegistrationOpen(false)}
            memberId={selectedMemberForQrCode?.MemberID}
            memberName={selectedMemberForQrCode?.FullName}
            onSuccess={handleQrCodeRegistrationSuccess}
          />
        </>
      )}
    </Box>
