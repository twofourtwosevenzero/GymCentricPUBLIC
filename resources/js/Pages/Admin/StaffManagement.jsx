...
export default function StaffManagement({
  ...
  //netpay modal
  const [isNetPayModalOpen, setIsNetPayModalOpen] = useState(false);
  const [netPayValue, setNetPayValue] = useState("0.00");
  ...
  const handleCalculateTotalNetPay = () => {
    ...
    setNetPayValue(total.toFixed(2));
    setIsNetPayModalOpen(true);
  };
    ...
    try {
      ...
      if (error.response && error.response.data && error.response.data.errors) {
        ...
        Object.keys(validationErrors).forEach(field => {
          ...
          // Handle time-related validation errors specially
          if (field === 'TimeOut' && messages.some(msg => msg.includes('after time in'))) {
  ...
  useEffect(() => {
    // Fetch admin info to get the default branch
    axios
      .get('/admin/info')
      .then((response) => {
        setAdminInfo(response.data.admin);
        
        // If admin has a default branch ID, set it as the default branch
    ...
    axios
      ...
      .then((res) => {
        ...
        // If only one branch (excluding "All Branches"), set it as default
        if (newBranchOptions.length === 2) {
          setBranch(newBranchOptions[1].value);
        }
      })
      .catch((err) => {
        console.error("Error fetching admin's branches:", err);
        // Set at least the "All Branches" option
        ...
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item>
            <FormControl size="small" sx={{ minWidth: 140 }}>
              ...
              <Select label="Branch" value={branch} onChange={handleBranchChange}>
            ...
            onAdd={(attendanceData) => {
              axios
                ...
                .then((res) => {
                  ...
                  showSuccessMessage("Attendance record created successfully!");
                })
                .catch((err) => {
                  ...
                  if (err.response && err.response.data && err.response.data.errors) {
                    ...
                    alert("Error creating attendance record. Please try again.");
                  }
                });
            }}
            staffOptions={staffRecords.map((s) => ({
          ...
          onSchedulesCreated={() => {
            axios.get(route("staff.schedules.index"))
              ...
              .catch((err) => {
                ...
                alert("Error fetching updated schedules. Please refresh the page.");
              });
          }}
          onError={(err) => {
...
>
  ...
  <DialogActions sx={{ justifyContent: "flex-end", gap: 2, py: 2 }}>
    ...
    >
      ...
      {isNetPayModalOpen && (
  <Dialog
    open={isNetPayModalOpen}
    onClose={() => setIsNetPayModalOpen(false)}
    fullWidth
    maxWidth="sm"
    sx={{
      "& .MuiDialog-paper": {
  ...
  >
    <DialogTitle>Total Net Pay</DialogTitle>
    <DialogContent dividers>
      <Typography variant="h6">
        Total Net Pay for the selected period: ₱{netPayValue}
      </Typography>
    </DialogContent>
    <DialogActions>
      <Button onClick={() => setIsNetPayModalOpen(false)} color="primary">
        Close
      </Button>
    </DialogActions>
  </Dialog>
